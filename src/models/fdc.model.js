...
/** Columnas de fdc_page_content (sin id) en el mismo orden que el upsert. */
function buildFdcPageContentUpsertValues(payload) {
  return [
    payload.heroEyebrow,
    payload.heroTitle,
...
const FDC_PAGE_CONTENT_COLUMNS = [
  'hero_eyebrow',
  'hero_title',
  'hero_subtitle',
  'hero_slogan',
...
export async function upsertFdcPageContentRow(payload) {
  const values = buildFdcPageContentUpsertValues(payload)
  if (values.length !== FDC_PAGE_CONTENT_COLUMNS.length) {
    throw new Error(
      `fdc_page_content upsert: se esperaban ${FDC_PAGE_CONTENT_COLUMNS.length} valores y llegaron ${values.length}`,
    )
  }

  const insertColumns = ['id', ...FDC_PAGE_CONTENT_COLUMNS].join(',\n      ')
  const insertPlaceholders = ['1', ...FDC_PAGE_CONTENT_COLUMNS.map(() => '?')].join(', ')
  const updateAssignments = FDC_PAGE_CONTENT_COLUMNS.map(
