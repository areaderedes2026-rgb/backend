...
function mapGastronomicCatalogRow(row) {
  if (!row) return null
  return {
    ...
    venues: parseJsonSafe(row.venues_json, []),
...
export async function getGastronomicCatalogContentRow() {
  const [rows] = await pool.query(
    'SELECT * FROM gastronomic_catalog_content WHERE id = 1 LIMIT 1',
  )
  return mapGastronomicCatalogRow(rows[0] ?? null)
}

export async function upsertGastronomicCatalogContentRow(payload) {
  await pool.query(
    `INSERT INTO gastronomic_catalog_content (
      id,
      hero_eyebrow,
      hero_title,
      hero_subtitle,
    ...
    ON DUPLICATE KEY UPDATE
      ...
      venues_json = VALUES(venues_json),
